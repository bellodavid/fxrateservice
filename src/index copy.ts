...
async function fetchBananaCrystalRates(
  ...
  // Calculate BananaCrystal rates for each currency
  allCurrencies.forEach((currency) => {
    const validRates = otherRates
      .filter((source) => source.rates[currency])
      .map((source) => ({
        buyRate: source.rates[currency].buyRate,
        sellRate: source.rates[currency].sellRate,
        midRate:
          (source.rates[currency].buyRate + source.rates[currency].sellRate) /
          2,
      }));
    ...
    if (validRates.length > 0) {
      ...
      const avgMidRate =
        validRates.reduce((sum, rate) => sum + rate.midRate, 0) /
        validRates.length;
      const avgSpread =
        validRates.reduce(
          (sum, rate) => sum + (rate.sellRate - rate.buyRate),
          0
        ) / validRates.length;

      // Set the BananaCrystal rate with the average spread
      const halfSpread = avgSpread / 2;
      aggregatedRates[currency] = {
        buyRate: avgMidRate - halfSpread,
        sellRate: avgMidRate + halfSpread,
      };
    }
  });
...
app.get("/api/all-bananacrystal-rates", async (req, res) => {
  try {
    ...
    // Find the BananaCrystal rates from the aggregated rates
    const bananaCrystalRates = rates.find(
      (rate) => rate.source === "BananaCrystal"
    );

    if (!bananaCrystalRates) {
      throw new Error("BananaCrystal rates not found in aggregated rates");
    }
    ...
    Object.entries(bananaCrystalRates.rates).forEach(([currency, rate]) => {
      if (currency !== BASE_CURRENCY) {
        // Calculate confidence and volatility based on other rates
        const otherSourceRates = rates
          .filter(
            (source) =>
              source.source !== "BananaCrystal" && source.rates[currency]
          )
          .map((source) => ({
            buyRate: source.rates[currency].buyRate,
            sellRate: source.rates[currency].sellRate,
        ...
        const avgBuyRate =
          otherSourceRates.reduce((sum, rate) => sum + rate.buyRate, 0) /
          otherSourceRates.length;
        const avgSellRate =
          otherSourceRates.reduce((sum, rate) => sum + rate.sellRate, 0) /
          otherSourceRates.length;

        const buyRateStdDev = Math.sqrt(
          otherSourceRates.reduce(
            (sum, rate) => sum + Math.pow(rate.buyRate - avgBuyRate, 2),
            0
          ) / otherSourceRates.length
        );
        const sellRateStdDev = Math.sqrt(
          otherSourceRates.reduce(
            (sum, rate) => sum + Math.pow(rate.sellRate - avgSellRate, 2),
            0
          ) / otherSourceRates.length
        );

        // Calculate confidence based on average of buy/sell rate variations
        const avgRate = (avgBuyRate + avgSellRate) / 2;
        const avgStdDev = (buyRateStdDev + sellRateStdDev) / 2;
        const confidence = Math.min(100, (1 - avgStdDev / avgRate) * 100);
        const volatilityIndex = (avgStdDev / avgRate) * 100;

        simplifiedRates[currency] = {
          buyRate: Number(rate.buyRate.toFixed(6)),
          sellRate: Number(rate.sellRate.toFixed(6)),
    ...
    res.json({
      base: BASE_CURRENCY,
      timestamp: bananaCrystalRates.timestamp,
      rates: simplifiedRates,
      metadata: {
        sourcesUsed: rates
          .filter((rate) => rate.source !== "BananaCrystal")
          .map((rate) => rate.source),
        totalCurrencies: Object.keys(simplifiedRates).length,
        lastUpdated: new Date(bananaCrystalRates.timestamp).toISOString(),
      },
    });
  } catch (error) {
    console.error("Error fetching BananaCrystal rates:", error);
    res.status(500).json({
